        ...
        for (event, elem) in context:
            if event == "start":
                ...
                if len(state_stack) <= 2 and elem.tag == self.NS + "drug":
                    ...
                    if "primary" in elem.attrib and state_stack[-3] == self.NS + "drugbank" and state_stack[
                        -2] == self.NS + "drug":
                        drug_id = None
                    elif len(state_stack) > 3 and state_stack[-3] == self.NS + "drug-interactions" and state_stack[
            ...
            if event == "end":
                ...
                if elem.tag == self.NS + "drugbank-id":
                    ...
                    elif state_stack[-2] == self.NS + "international-brand" and state_stack[
                    ...
                    if len(state_stack) > 3 and state_stack[-3] == self.NS + "drug-interactions" and state_stack[
                        -2] == self.NS + "drug-interaction":
                    ...
                    if state_stack[-3] == self.NS + "external-identifiers" and state_stack[
...
def get_disease_specific_drugs(parser, selected_drugs, phenotypes):
    ...
    for indication, diseases in indication_to_diseases.iteritems():
        if len(diseases) == 0:
            print
            indication.encode('ascii', 'ignore')
        elif indication.find(" not ") != -1 or indication.find(" except ") != -1:
            print
            diseases, indication.encode('ascii', 'ignore')
...
def output_drug_info(file_name, output_file):
    ...
    for drug, name in parser.drug_to_name.iteritems():
        ...
        try:
            ...
            print
            drug, name, groups, pubchem, description, indication, targets
            return
    f.close()
    return
