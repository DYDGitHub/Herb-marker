...
class TsvReader(FormattedFileProcessor):
    ...
    def __init__(self, input_file_name, delim="\t", inner_delim=None, quotation=None):
        FormattedFileProcessor.__init__(self, input_file_name=input_file_name, input_type="tsv", delim=delim,
                                        inner_delim=inner_delim, quotation=quotation)
        return
                ...
                if keys_to_include is None or id_ in keys_to_include:
                    ...
                    if merge_inner_values:
                        for x in fields_to_include:
                            new_vals.extend(map(lambda x: x.strip().strip(self.quotation),
                                                vals[columns[x]].split(self.inner_delim)))
                    else:
...
def get_from_to_mapping(file_name, from_column=None, to_column=None, delim="\t", inner_delim=None, filter_column=None,
                        exclude_value=None, include_value=None, one_to_one=False):
    """
    ...
    header_to_index, key_to_values = reader.read(fields_to_include=fields_to_include,
                                                 merge_inner_values=merge_inner_values)
    ...
    if len(header_to_index) > 2:
        ...
        print
        "Assuming the first column as FROM and the second column as TO fields!"
    from_to_id_mapping = {}
    for key, values in key_to_values.iteritems():
        ...
        if one_to_one:
            if len(vals) > 1:
                ...
                vals.sort(key=len)
                print
                "Selecting the first among shortest", vals
