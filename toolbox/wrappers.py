...
# e.g. 10/2015
#######################################################################
import toolbox.network_utilities as network_utilities
import stat_utilities, dict_utilities, text_utilities
...
def get_mapping(file_name, from_column=None, to_column=None, delim=None, one_to_one=True):
    ...
    """
    key_to_value = TsvReader.get_from_to_mapping(file_name, from_column=from_column, to_column=to_column, delim=delim,
                                                 inner_delim=None, filter_column=None, exclude_value=None,
                                                 include_value=None, one_to_one=one_to_one)
    return key_to_value
...
def get_mesh_id_mapping(desc_file, rel_file, dump_file=None):
    ...
    mesh_id_to_name, concept_id_to_mesh_id, mesh_id_to_name_with_synonyms = parse_umls.get_mesh_id_mapping(desc_file,
                                                                                                           rel_file,
                                                                                                           dump_file=dump_file)
...
def get_homology_mapping(homologene_file, tax_id="10090", from_tax_id="9606", symbol_type="geneid"):
    ...
    """
    geneid_to_geneid, group_to_taxid_to_geneid = parse_ncbi.get_homology_mapping(homologene_file, tax_id,
...
def calculate_lcc_significance(network, nodes, nodes_random=None, bins=None, n_random=1000, min_bin_size=100,
                               seed=452456):
...
def get_z_genes(file_name, cutoff_z=2):
    ...
    """
    gexp, gene_to_idx, cell_line_to_idx = get_expression_info(file_name, process=None, delim='\t',
                                                              R_header=True)  # , quote='"', dump_file=None)
...
def get_comorbidity_info(comorbidity_file, disease_ontology_file, mesh_dump, correlation_type="RR",
    ...
    mesh_id_to_name, concept_id_to_mesh_id, mesh_id_to_name_with_synonyms = get_mesh_id_mapping(None, None,
                                                                                                dump_file=mesh_dump)
...
def get_medi_indications(medi_file, drugbank_file, mesh_dump, disease_ontology_file, only_hps=True):
    ...
    mesh_id_to_name, concept_id_to_mesh_id, mesh_id_to_name_with_synonyms = get_mesh_id_mapping(None, None,
                                                                                                dump_file=mesh_dump)
    icd_to_mesh_ids = get_icd_to_mesh_ids(disease_ontology_file, id_type="ICD9CM")
    drug_to_indications = parse_medi.get_drug_disease_mapping(name_to_icd_and_confidences, name_to_drug,
...
def get_hetionet_indications(hetionet_file, mesh_dump, disease_ontology_file):
    ...
    mesh_id_to_name, concept_id_to_mesh_id, mesh_id_to_name_with_synonyms = get_mesh_id_mapping(None, None,
                                                                                                dump_file=mesh_dump)
    drug_to_indications = parse_hetionet.get_drug_disease_mapping(drug_to_do_ids, do_to_mesh_ids, mesh_id_to_name,
                                                                  dump_file=None)
    drug_to_diseases = {}
    for drug, values in drug_to_indications.iteritems():
        for phenotype, dui, val in values:
...
def calculate_proximity(network, nodes_from, nodes_to, nodes_from_random=None, nodes_to_random=None, bins=None,
    ...
    if distance != "closest":
        lengths = network_utilities.get_shortest_path_lengths(network, "temp_n%d_e%d.sif.pcl" % (
        len(nodes_network), network.number_of_edges()))
    ...
    pval2 = stats.mannwhitneyu(values, d, alternative='greater').pvalue
    m, s = numpy.mean(values), numpy.std(values)
    if s == 0:
        z = 0.0
    else:
        z = (d - m) / s
    return d, z, (m, s), pval, pval2


def calculate_proximity_multiple(network, from_file=None, to_file=None, n_random=1000, min_bin_size=100, seed=452456,
...
def calculate_separation_proximity(network, nodes_from, nodes_to, nodes_from_random=None, nodes_to_random=None,
                                   bins=None, n_random=1000, min_bin_size=100, seed=452456, lengths=None):
    """
    Calculate proximity from nodes_from to nodes_to
    If degree binning or random nodes are not given, they are generated
    lengths: precalculated shortest path length dictionary
...
def run_guild(phenotype, node_to_score, network_nodes, network_file, output_dir, executable_path=None,
              background_score=0.01, qname=None, method='s'):
    # Create node file
    ...
    if method == 's':
        n_repetition = 3
        n_iteration = 2
        score_command = ' -s s -n "%s" -e "%s" -o "%s" -r %d -i %d' % (
        node_file, network_file, output_file, n_repetition, n_iteration)
    elif method == 'd':
    ...
    if qname is None:
        ...
        print
        "%s" % (score_command.replace('"', ''))
    return score_command


def guildify_multiple(network_file, to_file, output_dir, from_file=None, out_file="guild.txt", method="s",
    ...
    for target, geneids in disease_to_genes.iteritems():
        ...
        if os.path.exists(node_file):
            print
            "Skipping existing:", node_file
            continue
        run_guild(target_mod, target_to_score, nodes, network_lcc_file, output_dir, executable_path,
                  background_score=0.01, qname="print", method=method)  # !
...
def get_scores(score_file):
    ...
    """
    nodes, dummy, node_to_score, dummy = network_utilities.get_nodes_and_edges_from_sif_file(file_name=score_file,
...
def check_functional_enrichment(id_list, background_id_weights=None, id_type="genesymbol", species="Homo sapiens",
                                mode="unordered", evidences=None, out_file_name=None, tex_format=False):
    """
    ...
    return functional_enrichment.check_functional_enrichment(id_list, background_id_weights, id_type, f_output,
                                                             species=species, mode=mode, tex_format=tex_format,
                                                             support=evidences, associations=None)
