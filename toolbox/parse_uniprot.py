...
def get_uniprot_to_geneid_from_idmapping_file(file_name, uniprot_ids=None):
    ...
    column_to_index, id_to_values = parser.read(fields_to_include=["UniProtKB-AC", "GeneID (EntrezGene)"],
                                                keys_to_include=uniprot_ids, merge_inner_values=True)
    uniprot_to_geneid = {}
...
class UniprotXMLParser(object):
    ...
    def parse(self):
        ...
        root.clear()
        print
        "Ignored mofications: ", ignored_modification_types
        return elements
