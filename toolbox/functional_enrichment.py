...
def check_functional_enrichment_of_human_gene_symbols(file_name, out_file_name):
    ...
    check_functional_enrichment(a, None, "genesymbol", open(out_file_name, 'w').write, species="Homo sapiens",
                                mode="unordered", request_info=False, tex_format=False)
    return


def check_functional_enrichment(subset_gene_ids, gene_weights, id_type, output_method=None, species="Homo sapiens",
                                mode="unordered", request_info=False, tex_format=False, support=None,
                                associations=None):
    """
    ...
    if id_type == "geneid":
        ...
        if species == "Homo sapiens":
            ...
            print
            client_funcassociate.available_namespaces(species=[species])
...
def get_functional_enrichment(enrichment_file, go, remove_parents=False, only_biological_processes=False,
                              only_slim=False, logodds_cutoff=0):
    """
    ...
    for line in f:
        ...
        if line.startswith("# of"):
            ...
            if only_biological_processes:
                if g.node[go_term][
                    't'] == "biological_process":  # and 'a' in g.node[go_term]: #is bp and slim # ("molecular_function", "biological_process"):
                    if only_slim:
