...
class UMLS(object):
    ...
    def _get_concept_info(self, concept_types=None, concept_sources=None, only_preferred=False):
        ...
        if self.concept_id_to_values is None and self.concept_to_concept_id is None:
            ...
            header_names = ["CUI", "LAT", "TS", "LUI", "STT", "SUI", "ISPREF", "AUI", "SAUI", "SCUI", "SDUI", "SAB",
                            "TTY", "CODE", "STR", "SRL", "SUPPRESS", "CVF", "dummy"]
            ...
            for line in f:
                ...
                if concept_id in self.concept_to_concept_id:
                    print
                    "Concept id conflict - overwriting:", concept, self.concept_to_concept_id[concept], concept_id
    ...
    def get_relations(self, relation_types=None, relation_a_types=None,
                      source_types=None):  # , "may_treat", "may_be_treated"
        """
        ...
        if self.concept_id_to_relations is None:
            ...
            header_names = ["CUI1", "AUI1", "STYPE1", "REL", "CUI2", "AUI2", "STYPE2", "RELA", "RUI", "SRUI", "SAB",
                            "SL", "RG", "DIR", "SUPPRESS", "CVF", "dummy"]
            col_to_idx = dict((val.lower(), i) for i, val in enumerate(header_names))
            for line in f:
                words = line.strip("\n").split(self.delim)
    ...
    def get_drug_disease_relations(self):
        ...
        for nodes in self.get_ontology(root_concept="Pharmaceutical / biologic product", relation_a_types=set(["isa"]),
                                       source_types=set(["SNOMEDCT"])).edges():
            for node in nodes:
                try:
...
def get_basic_info(desc_file, rel_file):
    ...
    print
    concept_id, g.edges([concept_id])
    for s, v in g.edges([concept_id]):
        print
        s, v, u.get_concepts(v, concept_sources=sources)
