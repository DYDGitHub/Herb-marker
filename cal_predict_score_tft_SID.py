...
def shortest_distance_zscore(df, tft, herb_target_dict, symptom_target_dict):
    predict_scores = []
    for index, row in df.iterrows():
        ...
        if row['SID'] in symptom_target_dict:
            ...
            predict_scores.append(
                [row['herb_UID'], row['SID'], tft.distance_zscore(dts, hts, 10), row['clinical occurrence'],
...
if __name__ == '__main__':
    ...
    split = num_pairs / 20
    pool = mp.Pool(20)
    results = [pool.apply_async(shortest_distance_zscore, args=(df1[int(i * split): int((i + 1) * split)], tft,
    ...
    df.to_csv('results/predict_scores_%s_tft_SID.csv' % file_name)
